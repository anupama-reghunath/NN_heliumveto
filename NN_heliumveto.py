...
def print_features(inputmatrix,truth,prefix="Features_"):
    ...
    for i,tag in enumerate(["Signal+MuinducedBG","neuDIS+MuinducedBG","muDIS+MuinducedBG"]):
        plt.hist(inputmatrix[truth == i][:,856],bins=100, range=[-2800, 2800], histtype='step',linewidth=3,label=tag,color=colors[tag])
        plt.ylabel('count')
        plt.legend()
        #plt.yscale("log")
    #plt.ylim(1,10000)
...
def plot_scalingeffect(inputmatrix,inputmatrix_scaled,truth):
    ...
    for i,tag in enumerate(["Signal+MuinducedBG","neuDIS+MuinducedBG","muDIS+MuinducedBG"]):
        x_vertex = inputmatrix_scaled[truth == i][:, 854]
        y_vertex = inputmatrix_scaled[truth == i][:, 855]
    ...
    for i,tag in enumerate(["Signal+MuinducedBG","neuDIS+MuinducedBG","muDIS+MuinducedBG"]):
        plt.hist(inputmatrix_scaled[truth == i][:,856],bins=100, histtype='step',linewidth=3,label=tag,color=colors[tag])
        plt.ylabel('count')
        plt.legend()
        plt.yscale("log")
    #plt.ylim(1,10000)
...
def NN_training(model,epochs=30,prefix=''):
...
X_withthreshold[:, :854] = np.where(X[:, :854] > threshold * 0.001, X[:, :854], 0)#Set SBT hit Edep to 0 if less than threshold
...
truth = Y[mask]
print(f"\nnEvents with Edep > {threshold} MeV threshold = {len(inputmatrix_full)}")

#Limiting the data set to use only Edep no Timing!
inputmatrix = inputmatrix_full[:, :857]         #without signal info,len(UBT_hits),event_weight in 15 years
signal_info = inputmatrix_full[:, 854:]         #vertex x, y, z,signal info,len(UBT_hits),event_weight in 15 years
...
scaler = RobustScaler()
inputmatrix_scaled = scaler.fit_transform(inputmatrix)
joblib.dump(scaler, "robust_scaler.pkl")

# Save scaler parameters for reference
scaler_params = {'center_': scaler.center_.tolist(), 'scale_': scaler.scale_.tolist()}
...
model = NN(input_dim=input_dim, output_dim=output_dim, hidden_sizes=hidden_sizes, dropout=0.2)

#class_weights   = torch.tensor([1.0, 1.0, 1.0])# Emphasize Signal caution
criterion       = nn.CrossEntropyLoss()#weight=class_weights) #reduction=False (?) loss=torch.mean(weights*loss)
...
model_path=NN_training(model,epochs=100,prefix='')# Define hyperparameters
...
with torch.no_grad():
    for inputs, labels, signal in test_loader:
        ...
        probabilities = softmax(outputs, dim=1)  # Convert logits to probabilities
        _, predicted = torch.max(probabilities, 1)  # Get class with highest probability
